...
function shortName(s) { return s ? (s.length > 20 ? s.slice(0, 18) + "…" : s) : "—"; }
function shortProp(s) { return s ? (s.length > 18 ? s.slice(0, 16) + "…" : s) : "—"; }
...
function buildCache(getData) {
  ...
  if (criticalOpen.length > 0) {
    const top = criticalOpen.slice(0, 5);
    criticalText += "\n\n| Reference | Property | Vendor |\n|---|---|---|";
    top.forEach(r => {
      criticalText += `\n| ${r["Reference"]} | ${shortProp(r["Property"])} | ${shortName(r["Vendor"])} |`;
    });
    if (criticalOpen.length > 5) criticalText += `\n\n...and ${criticalOpen.length - 5} more. Ask me to filter by region, property, or FM.`;
  }
  ...
  let vendorText = "**Top 10 vendors by spend:**\n\n| Vendor | WOs | Spend |\n|---|---|---|";
  topVendors.forEach(([v, d]) => {
    vendorText += `\n| ${shortName(v)} | ${d.count} | ${fmtCurrency(d.spend)} |`;
  });
  ...
  if (exceeded.length > 0) {
    const top = exceeded.sort((a, b) => (parseNum(b["Total"]) - parseNum(b["Not To Exceed"])) - (parseNum(a["Total"]) - parseNum(a["Not To Exceed"]))).slice(0, 5);
    nteText += "\n\n| Reference | Property | Over By |\n|---|---|---|";
    top.forEach(r => {
      const nte = parseNum(r["Not To Exceed"]);
      const actual = parseNum(r["Total"]);
      nteText += `\n| ${r["Reference"]} | ${shortProp(r["Property"])} | ${fmtCurrency(actual - nte)} |`;
    });
    if (exceeded.length > 5) nteText += `\n\n...and ${exceeded.length - 5} more.`;
  }
